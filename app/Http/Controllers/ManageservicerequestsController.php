...
use App\Models\Manageservicerequests;
use Illuminate\Http\Request;
use Illuminate\Pagination\LengthAwarePaginator as Paginator;
use Validator, Input, Redirect;

class ManageservicerequestsController extends Controller
{
    ...
    {
        parent::__construct();
        $this->model = new Manageservicerequests();
        ...
        $this->data = array(
            ...
            'pageModule' => 'manageservicerequests',
            'pageUrl' => url('manageservicerequests'),
            'return' => self::returnUrl()
        );
        $this->sortMapping = ['requestor_id' => 'u1.username', "location_id" => "location.location_name", 'status_id' => 'service_status.status'];
        $this->sortUnMapping = ['u1.username' => 'requestor_id', "location.location_name" => "location_id", 'service_status.status' => 'status_id'];
    ...
    public function getIndex()
    {
        ...
        return view('manageservicerequests.index', $this->data);
    }
...
// Render into template
        return view('manageservicerequests.table', $this->data);
    ...
    function getUpdate(Request $request, $id = null)
    {
        ...
        if ($row) {
            ...
            $this->data['row'] = $this->model->getColumnTable('service_requests');
        }
        $this->data['setting'] = $this->info['setting'];
        ...
        return view('manageservicerequests.form', $this->data);
    }
    ...
    function postCopy(Request $request)
    {
        ...
        foreach (\DB::select("SHOW COLUMNS FROM service_requests ") as $column) {
            if ($column->Field != 'id')
        ...
        $sql = "INSERT INTO service_requests (" . implode(",", $columns) . ") ";
        $sql .= " SELECT " . implode(",", $columns) . " FROM service_requests WHERE id IN (" . $toCopy . ")";
        \DB::insert($sql);
    ...
    function getTest()
    {
        $rows = \DB::select('select id,attachment_path from service_requests');
        $count = 0;
        foreach ($rows as $r) {
            if (!empty($r->attachment_path)) {
                $count++;
                $file = strrchr($r->attachment_path, '/');
                $file = substr(1, $file . length, $file);
                \DB::update("update service_requests set attachment_path='" . $file . "' where id=" . $r->id);
            }
