...
use App\Models\Feg\System\SystemEmailReportManager;
use Illuminate\Http\Request;
use Illuminate\Pagination\LengthAwarePaginator as Paginator;
use Validator, Input, Redirect, Session, Auth, DB; 

class SystemEmailReportManagerController extends Controller
{

	protected $layout = "layouts.main";
	protected $data = array();	
	public $module = 'systememailreportmanager';
	static $per_page	= '20';
	
	public function __construct() 
	{
		parent::__construct();
		$this->model = new Systememailreportmanager();
		...
		$this->data = array(
			...
			'pageModule'		=> 'systememailreportmanager',
			'pageUrl'			=>  url('feg/system/systememailreportmanager'),
    ...
    public function postData(Request $request)
    {
        ...
        $module_id = \DB::table('tb_module')->where('module_name', '=', 'systememailreportmanage')->pluck('module_id');
		...
		$pagination->setPath('systememailreportmanager/data');
    ...
    function getUpdate(Request $request, $id = null)
    {
		...
		if($row)
		{
            ...
            $this->data['row'] = $this->model->getColumnTable('system_email_report_manager');
        }
        $this->data['setting'] = $this->info['setting'];
    ...
    function postCopy(Request $request)
    {
	    ...
	    foreach(\DB::select("SHOW COLUMNS FROM system_email_report_manager ") as $column)
        {
            if ($column->Field != 'id')
        ...
        $sql = "INSERT INTO system_email_report_manager (" . implode(",", $columns) . ") ";
        $sql .= " SELECT " . implode(",", $columns) . " FROM system_email_report_manager WHERE id IN (" . $toCopy . ")";
        \DB::insert($sql);
...
//		if ($validator->passes()) {
			$data = $this->validatePost('system_email_report_manager');
            $arrayFields = array(
                'to_email_groups',
                'to_email_individuals',
                'cc_email_groups',
                'cc_email_individuals',
                'bcc_email_groups',
                'bcc_email_individuals',
                );
