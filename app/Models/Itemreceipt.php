...
//echo $select . " {$params} " . self::queryGroup() . " {$orderConditional}  {$limitConditional} ";
    ...
    public static function addOrderReceiptItems($data,$param=null){
        ...
        if(empty($qry_in_string))
            $qry_in_string = "''";
        $order_received_data=\DB::select("select order_received.*,order_contents.qty_per_case from order_received inner join order_contents on order_contents.id=order_received.order_line_item_id where order_id in($qry_in_string) $where");
