...
class gamesintransit extends Sximo
{
    ...
    public static function querySelect()
    {
        ...
        return "SELECT game.*,IF(game.test_piece = 1, CONCAT('**TEST** ',game_title.game_title),game_title.game_title) as game_name,game.game_name as gname,
               ...
               IF(game.intended_first_location IS NULL OR game.intended_first_location=0, '<em style=\"color:red\">Check Notes to Confirm</em>', CONCAT(L3.id, ' | ',L3.location_name_short)) as intended_first_location,game_title.img,
               IF(game_title.game_title LIKE '%*TEST*%','', CONCAT('../qr/',game.id,'.png')),If(L2.id IS NULL, 'NEW GAME' , CONCAT(L2.id,' | ',L2.location_name_short)) ,
               game_status.game_status,If(Y.game_type IS NULL, '' , Y.game_type) ,If(V.vendor_name IS NULL, '' , V.vendor_name),game_title.game_type_id,game_title.img
               From game
			   ...
			   LEFT JOIN game_type Y ON (game_title.game_type_id = Y.id)
			   LEFT JOIN vendor V ON (game_title.mfg_id = V.id)";
    }
