    ...
    public static function getRows($args, $cond = null) {
        ...
        if(isset($_SESSION['managefegrequeststore_search'])&& !empty($_SESSION['managefegrequeststore_search'])){
            ...
            if($pre_products['managefegrequeststore']!=null){
                ...
                $products=$products->filter(function ($item) use($request_location_id,$product_id_not_in_final,$product_type_id_not_in_final)
                {
                    if(in_array($item->location_id,$request_location_id)
                        && (!in_array($item->product_id,$product_id_not_in_final)) && (!in_array($item->product['prod_type_id'],$product_type_id_not_in_final))){
                        return $item;}
                });
