...
class Sximo extends Model {
    ...
    function getMoveHistory($asset_id = null) {
        $query = \DB::table('game_move_history')
                ...
                ->leftJoin('game_title as gt', 'g.game_title_id', '=', 'gt.id')
                ->select('game_move_history.*', 'gt.game_title', 'CONCAT(u1.first_name," ",u1.last_name) as from_name', 'CONCAT(u2.first_name," ",u2.last_name) as to_name', 'l1.location_name as from_location','l1.id as from_location_id', 'l2.location_name as to_location','l2.id as to_location_id');
        if (!is_null($asset_id)) {
