...
(function(a) {
    a.fn.jCombo = function(b, d) {
        function h(b, d, e, f, g,excludeItems,displayonly,isTypeRestricted) {
            a.ajax({
                type: "GET",
                dataType: "json",
                url: d + e,
                success: function(a) {
                    ...
                    if (a.length == 0) {
                        ...
                        if(isTypeRestricted == false && displayonly.length < 1) {
                            if (f != "" && f != null) {
                        ...
                        for (var e = 0; e < a.length; e++) {
                          ...
                          // selected =  $.inArray(a[e][0], curr_values) ? ' selected="selected"' : "";
                            c = a[e];
                          if(excludeItems){
                              var preventedItem = false;
                              for(var exl=0; exl<excludeItems.length; exl++){
                                  if( c[0] == excludeItems[exl]){
                                      preventedItem = true;
                                  }
                              }
                              if(preventedItem === false){
                                  c = a[e];
                                  d += '<option value="' + c[0] + '"' + selected + ">" + c[1] + "</option>"
                              }
                          }else if(isTypeRestricted == true && displayonly){
                              var preventedItem = false;
                              for(var exl=0; exl<displayonly.length; exl++){
                                  if( c[0] == displayonly[exl]){
                                      preventedItem = false;
                                  }else{
                                      preventedItem = true;
                                  }
        ...
        h(f, b, d.parent_value, d.initial_text, d.selected_value,d.excludeItems,$.trim(d.displayonly),d.isTypeRestricted)
    }
})(jQuery)