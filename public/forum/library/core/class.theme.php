   ...
   public static function Link($Path, $Text = FALSE, $Format = NULL, $Options = array()) {
      ...
      switch ($Path) {
         ...
         case 'signinout':
            ...
            if ($Session->IsValid()) {
               if(!$Text)
                  $Text = T('Back to Admin');//element5 15 May 2017
               $Path =  (isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] === 'on' ? 'https://' : 'http://').$_SERVER['HTTP_HOST']; //SignOutUrl($Target);//element5 15 May 2017
