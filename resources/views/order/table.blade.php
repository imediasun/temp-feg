			...
			<tr id="form-0" >
			  ...
			  @endif        
           		...
           		<?php foreach ($rowData as $row) : 
           			  $id = $row->orderNumber;
           		?>
                <tr class="editable" id="form-{{ $row->orderNumber }}">
					<td class="number"> <?php echo ++$i;?>  </td>
					<td ><input type="checkbox" class="ids" name="ids[]" value="<?php echo $row->orderNumber ;?>" />  </td>					
					@if($setting['view-method']=='expand')
					<td><a href="javascript:void(0)" class="expandable" rel="#row-{{ $row->orderNumber }}" data-url="{{ url('order/show/'.$id) }}"><i class="fa fa-plus " ></i></a></td>								
					@endif			
					 <?php foreach ($tableGrid as $field) :
						...
						endforeach; 
					  ?>
				 <td data-values="action" data-key="<?php echo $row->orderNumber ;?>">
					{!! AjaxHelpers::buttonAction('order',$access,$id ,$setting) !!}
					{!! AjaxHelpers::buttonActionInline($row->orderNumber,'orderNumber') !!}		
				</td>			 
                </tr>
                @if($setting['view-method']=='expand')
                <tr style="display:none" class="expanded" id="row-{{ $row->orderNumber }}">
                	<td class="number"></td>
                	<td></td>
                	<td></td>
