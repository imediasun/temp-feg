...
@if($setting['form-method'] =='native')
    ...
    <div class="sbox">
        <div class="sbox-title">
            <h4>@if($id)
                ...
                <a href="javascript:void(0)" class="collapse-close pull-right btn btn-xs btn-danger"
                   onclick="ajaxViewClose('#{{ $pageModule.'ItemForm' }}'); $('#orderView').show('slow');"><i
        ...
        <div class="sbox-content">
            @endif
            @include('product.form.form',['actionUrl' => $actionUrl, 'fields' => $fields, 'row' => $row, 'variations' => $variations, 'productTypes' => $productTypes, 'id' => $id,'pageModule' => $pageModule, 'fromOrder' => $fromOrder, 'rowId' => $rowId,'itemForm' => 'ItemForm' ])
            @if($setting['form-method'] =='native')
        </div>
    </div>
@endif
...
<script type="text/javascript">
    ...
    $(document).ready(function () {
        ...
        getExpenseCategory('{{ $variation["prod_type_id"] }}', null,{{ $variationCount }});
        <?php
            $variationCount++;
        ?>
        @endforeach
        ...
        $('.ajaxLoading').hide();
        if ('{{ $row["prod_type_id"] }}') {
            $("#prod_sub_type_id_1").jCombo("{{ URL::to('order/comboselect?filter=product_type:id:type_description') }}&parent=request_type_id:{{ $row["prod_type_id"] }}",
                    {selected_value: '{{ $row["prod_sub_type_id"] }}'});
        }
        $("#expense_category_1").jCombo("{{ URL::to('order/expense-category-groups') }}",
                {selected_value: '{{ $row["expense_category"] }}'});
        @endif
        $("#vendor_id[productVendor='1']").jCombo("{{ URL::to('vendor/comboselect?filter=vendor:id:vendor_name:hide:0:status:1') }}",
                {
        ...
        $('.fixDecimal[name="case_price"]').val($('.fixDecimal[name="case_price"]').fixDecimal());
        $('.fixDecimal[name="unit_price"]').val($('.fixDecimal[name="unit_price"]').fixDecimal());
        ...
        $(document).on('change', "select.prod_type", function () {
            ...
            if (selectedType && selectedType != previous) {
                ...
                console.log('Debug Product Type');
