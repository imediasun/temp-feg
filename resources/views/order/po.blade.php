...
<div style="position:relative">
    ...
    <div style="width:50%; position:absolute; left:50%;top:8px">
        <table  width="100%">
            <tr>
                <td style="border:1px solid #000;text-align: center;padding:8px 0px;">Date: <br/> {{ $data[0]['date_ordered'] }} </td>
                <td style="border:1px solid #000;text-align: center;padding:8px 0px;">PO # <br/>   {{ $data[0]['po_number'] }} </td>
            </tr>
        </table>
    </div>
    <div >
...
<br/>
    ...
    <div >
        <table width="100%" border="1" >
            <tr>
                <td style="padding:8px;text-align: left"> Ordered By : {{ $data[0]['first_name'] }} {{ $data[0]['last_name'] }} <br/>
                     Order Description
                </td>
            ...
            <tr>
                <td style="padding:8px;">
                    @foreach($data[0]['item_description_string'] as $description)
                {{ $description }} <br/>
                    @endforeach
                </td>
                <td style="padding:8px;">    @foreach($data[0]['item_price_string'] as $price)
                        {{ $price }} <br/>
                    @endforeach </td>
                <td style="padding:8px;">   @foreach($data[0]['item_qty_string'] as $qty)
                        {{ $qty }} <br/>
                    @endforeach</td>
                <td style="padding:8px;">   @foreach($data[0]['item_total_string'] as $total)
                        {{ $total }} <br/>
                    @endforeach</td>

            </tr>
            <tr style=""><td colspan="3" style="text-align: center;padding:8px">Shipping Method: {{ $data[0]['freight_type'] }}</td><td style="padding:8px">Order Total:<br/>$  {{ number_format($data[0]['order_total_cost'],2) }}</td></tr>
            @else
