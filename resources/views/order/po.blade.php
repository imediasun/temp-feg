            ...
            <table width="100%"  style="border-collapse: collapse;overflow:hidden;">
                ...
                @for($i=0;$i < count($data[0]['orderDescriptionArray']);$i++)
                    <tr>
                        ...
                        <td style="padding:2px;border:1px dotted  #000;border-top:none;border-right:1px dotted #000;text-align: right ">  {{ CurrencyHelpers::formatPrice(($data[0]['brokenCaseArray'][$i] && !in_array($data[0]['order_type_id'],$case_price_categories)) ? $data[0]['OriginalUnitPriceArray'][$i]* $data[0]['orderQtyArray'][$i] : $data[0]['OriginalCasePriceArray'][$i]* $data[0]['orderQtyArray'][$i],\App\Models\Order::ORDER_PERCISION) }}  <br/></td>
                        <td></td>
                    </tr>
                @endfor
                <tr style="">
