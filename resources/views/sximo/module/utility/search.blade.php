...
@foreach ($tableForm as $t)
	@if($t['search'] =='1')
		...
		<tr id="{{ $t['field'] }}" class="fieldsearch">
			...
			<td style="position: relative;" id="field_{{ $t['field']}}" width="60%">{!! SiteHelpers::transForm($t['field'] , $tableForm , false, '', $typeRestricted,false,true) !!}</td>
...
if('{{ $pageModule }}' == 'new-location-setup'){
	...
	$('#new-location-setupSearch select[name="location_id"]').jCombo(
			"{{ URL::to('new-location-setup/comboselect?filter=location:id:location_name|id ') }}"+ "&delimiter=%20-%20&space=1");
