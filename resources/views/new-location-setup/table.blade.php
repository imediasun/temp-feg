...
<div class="sbox">
    <div class="sbox-title">
        ...
        <div class="sbox-tools">
            <a href="javascript:void(0)" class="btn btn-xs btn-white tips" title="Clear Search"
               onclick="reloadData('#{{ $pageModule }}','new-location-setup/data?search=')"><i
                        class="fa fa-trash-o"></i> Clear Search </a>
            <a href="javascript:void(0)" class="btn btn-xs btn-white tips" title="Reload Data"
               onclick="reloadData('#{{ $pageModule }}','new-location-setup/data?return={{ $return }}')"><i
    ...
    <div class="sbox-content">
        ...
        <?php echo Form::open(array('url' => 'new-location-setup/delete/', 'class' => 'form-horizontal', 'id' => 'SximoTable', 'data-parsley-validate' => ''));?>
                    ...
                    <tr class="editable" id="form-{{ $row->id }}" data-id="{{ $row->id }}" id="form-{{ $row->id}}"
                        ...
                        @if($setting['view-method']=='expand')
                            <td><a href="javascript:void(0)" class="expandable" rel="#row-{{ $row->id }}"
                                   data-url="{{ url('new-location-setup'.$id) }}"><i class="fa fa-plus "></i></a></td>
                        @endif
                        ...
                        @if($setting['disablerowactions']=='false')
                            <td data-values="action" data-key="<?php echo $row->id;?>">
                                {!! AjaxHelpers::buttonAction('new-location-setup',$access,$id ,$setting) !!}
                            </td>
                        @endif
                    </tr>
                    @if($setting['view-method']=='expand')
