...
@if($setting['form-method'] =='native')
    <div class="sbox">
        ...
        <div class="sbox-content">
            ...
            <div class="col-md-12">
                <fieldset>
                    <legend> <?php if (empty($row['TicketID'])) echo 'Create A Ticket'; else echo 'Edit A Ticket'; ?></legend>
                    ...
                    <div class="form-group  ">
                        ...
                        <div class="col-md-6">
                            ...
                            <select name='issue_type' rows='5' required class='select2 '>
                                <?php
                    ...
                    <div class="form-group  ">
                        <label for="Debit Card" class=" control-label col-md-1 text-left">
                            {!! SiteHelpers::activeLang('Debit Card', (isset($fields['debit_card']['language'])? $fields['debit_card']['language'] : array())) !!}
                        </label>
                        <div class="col-md-6">
                            {!! Form::text('debit_card', $row['debit_card'],array('class'=>'form-control', 'id' => 'debit_card_id')) !!}
                        </div>
                        <div class="col-md-2">
                    ...
                    <div class="form-group  ">
                        ...
                        <div class="col-md-6">
                            ...
                            <a href="javascript:void(0)" class="btn btn-xs btn-primary pull-right"
                               onclick="addMoreFiles('file_path')"><i class="fa fa-plus"></i></a>
                                ...
                                @foreach($row['file_path'] as $files)
                                    @if(file_exists('./'.$files) && $files !='')
                                        <li id="cr-<?php echo $cr;?>" class="" style="margin-top: 1%; ">
                                            ...
                                            <span class="pull-right" rel="cr-<?php echo $cr;?>"
                                                  onclick=" $(this).parent().remove();"><i
                                                        class="fa fa-trash-o  btn btn-xs btn-danger"></i></span>
            ...
            $("#location_id").change(function(){
                ...
                if(location_id!="")
                {
                    $.ajax({
                        type: "GET",
                        dataType: "json",
                        url: 'location/comboselect?filter=location:debit_type_id:&parent=id:'+location_id,
                        success: function(a) {
                            if(a[0][0] == 1)
                            {
                                $("#debit_card_id").attr('value', 'Sacoa');
                            }else if(a[0][0] == 2){
                                $("#debit_card_id").attr('value','Embed');
                            }else {
                                $("#debit_card_id").attr('value','');
                            }
                        },
                        error: function(err){
                        }
