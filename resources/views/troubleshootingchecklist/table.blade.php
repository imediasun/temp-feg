...
<div class="sbox">
	<div class="sbox-title">
		...
		<div class="sbox-tools" >
			{{--<a href="javascript:void(0)" class="btn btn-xs btn-white tips" title="Clear Search" onclick="reloadData('#{{ $pageModule }}','troubleshootingchecklist/data?search=')"><i class="fa fa-trash-o"></i> Clear Search </a>--}}
			{{--<a href="javascript:void(0)" class="btn btn-xs btn-white tips" title="Reload Data" onclick="reloadData('#{{ $pageModule }}','troubleshootingchecklist/data?return={{ $return }}')"><i class="fa fa-refresh"></i></a>--}}
	...
	<div class="sbox-content">
	 ...
	 <?php echo Form::open(array('url'=>'troubleshootingchecklist/delete/', 'class'=>'form-horizontal' ,'id' =>'SximoTable'  ,'data-parsley-validate'=>'' )) ;?>
<div class="table-responsive">
    @if(!empty($topMessage))
    <h5 class="topMessage">{{ $topMessage }}</h5>
    @endif
	@if(count($rowData)>=1)
    <table class="table table-striped datagrid " style="min-width: auto !important;" id="{{ $pageModule }}Table" data-module="{{ $pageModule }}" data-url="{{ $pageUrl }}">
        <thead>
			<tr>
				...
				<th width="5%"><?php echo Lang::get('core.btn_action') ;?></th>
                @endif
			  </tr>
        </thead>
			...
			<tr id="form-0" >
			  ...
			  @endif
                ...
                <tr class="editable" id="form-{{ $row->id }}" data-id="{{ $row->id }}"  @if($setting['inline']!='false' && $setting['disablerowactions']=='false') ondblclick="showFloatingCancelSave(this)" @endif>
					...
					<td><a href="javascript:void(0)" class="expandable" rel="#row-{{ $row->id }}" data-url="{{ url('troubleshootingchecklist/show/'.$id) }}"><i class="fa fa-plus " ></i></a></td>
					@endif
				 ...
				 <td data-values="action" data-key="<?php echo $row->id ;?>">
					{!! AjaxHelpers::buttonAction('troubleshootingchecklist',$access,$id ,$setting) !!}
                 </td>
                @endif
                </tr>
                @if($setting['view-method']=='expand')
	...
	$( ".ui-sortable" ).sortable(
			{
				...
				update: function (event, ui) {
					...
					$.ajax({
						data: data,
						type: 'POST',
						url: '/troubleshootingchecklist/reordergrid',
