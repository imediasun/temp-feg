        ...
        <select class="form-control float-margin height-set" style="width:auto!important;display:inline;" name="col-config"
            ...
            @endforeach
                ...
                @if(\Session::get('uid') ==  \SiteHelpers::getConfigOwner($config_id))
                    ...
                    <button id="delete-cols" href="{{ URL::to('tablecols/arrange-cols/'.$pageModule.'/delete') }}" class=" float-marginbtn btn-sm btn-white tips" title="Delete column arrangement">  <i class="fa fa-trash-o"></i></button>
                @endif
            @endif
        @endif
    </div>
