...
<div class="sbox">
	<div class="sbox-title">
		...
		<div class="sbox-tools" >
			<a href="javascript:void(0)" class="btn btn-xs btn-white tips" title="Clear Search" onclick="reloadData('#{{ $pageModule }}','productsindevelopmentreport/data?search=')"><i class="fa fa-trash-o"></i> Clear Search </a>
			<a href="javascript:void(0)" class="btn btn-xs btn-white tips" title="Reload Data" onclick="reloadData('#{{ $pageModule }}','productsindevelopmentreport/data?return={{ $return }}')"><i class="fa fa-refresh"></i></a>
	...
	<div class="sbox-content">
	 ...
	 <?php echo Form::open(array('url'=>'productsindevelopmentreport/delete/', 'class'=>'form-horizontal' ,'id' =>'SximoTable'  ,'data-parsley-validate'=>'' )) ;?>
    @if(!empty($topMessage))
			...
			<tr id="form-0" >
			  ...
			  @endif
           		...
           		<?php foreach ($rowData as $row) :
           			  $id = $row->id;
           		?>
			<tr @if($access['is_edit']=='1') class="editable" @endif id="form-{{ $row->id }}" data-id="{{ $row->id }}"
				@if($setting['inline']!='false' && $setting['disablerowactions']!='false') @if($access['is_edit']=='1') ondblclick="showFloatingCancelSave(this)" @endif @endif>
					...
					<td><a href="javascript:void(0)" class="expandable" rel="#row-{{ $row->id }}" data-url="{{ url('productsindevelopmentreport/show/'.$id) }}"><i class="fa fa-plus " ></i></a></td>
					@endif
				 ...
				 <td data-values="action" data-key="<?php echo $row->id ;?>">
					{!! AjaxHelpers::buttonAction('productsindevelopmentreport',$access,$id ,$setting) !!}
