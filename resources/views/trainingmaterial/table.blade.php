...
<div class="sbox">
	...
	@if(count($rowData)>=1)
            <div style="padding-top: 20px; padding-bottom: 50px; background-color: #ffffff;text-align: justify">
                <h2 class="text-center">Training Videos</h2>
                <hr/>
               @foreach ($rowData as $row)
				   <div class="col-md-6">
					   <p><strong>{{ $row->video_title }} </strong><span style="color: #808080;">by {{ $row->users }}</span></p>
					   <p><iframe src="https://youtube.com/embed/{{$row->video_path}}" width="100%" height="380" allowfullscreen="allowfullscreen"></iframe></p>
				   </div>
                @endforeach
				<div class="col-md-12">
