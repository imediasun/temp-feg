<div class="row c-margin">
	<div class="col-md-9">
        ...
        @if($access['is_add'] ==1)
        <a href="javascript://ajax" class="btn btn-sm btn-white alignment-left-fixed" onclick="ajaxCopy('#{{ $pageModule }}','{{ $pageUrl }}')"><i class="fa fa-file-o"></i> Copy </a>
        @endif
        ...
        @endif
        @endif
        <a href="{{ URL::to( $pageModule .'/search') }}" class="btn btn-sm btn-white alignment-left-fixed" onclick="SximoModal(this.href,'Advanced Search'); return false;" ><i class="fa fa-search"></i>Advanced Search</a>
        ...
        <select class="form-control alignment-left-fixed" style="width:auto!important;display:inline;" name="col-config"
            ...
            @endforeach
                    ...
                    @if(!empty($config_id))
                        <a href="{{ URL::to('tablecols/arrange-cols/'.$pageModule.'/edit') }}" class="btn btn-sm btn-white alignment-left-fixed"
	...
	<div class="col-md-3 ">
		...
		<div class="pull-right">
			...
			@endif
            @if($isPrint)
			<a href="{{ URL::to( $pageModule .'/export/print?exportID='.uniqid('print', true).'&return='.$return) }}" class="btn btn-sm btn-white alignment-right-fixed" onclick="ajaxPopupStatic(this.href); return false;" > Print</a>
			@endif
		</div>	
		@endif
	</div>
