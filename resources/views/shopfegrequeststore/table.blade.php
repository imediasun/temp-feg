...
<div class="sbox">
	<div class="sbox-title">
		...
		<div class="sbox-tools" >
			<a href="javascript:void(0)" class="btn btn-xs btn-white tips" title="Clear Search" onclick="reloadData('#{{ $pageModule }}','shopfegrequeststore/data?search=')"><i class="fa fa-trash-o"></i> Clear Search </a>
			<a href="javascript:void(0)" class="btn btn-xs btn-white tips" title="Reload Data" onclick="reloadData('#{{ $pageModule }}','shopfegrequeststore/data?return={{ $return }}')"><i class="fa fa-refresh"></i></a>
			...
			<tr id="form-0" >
			  ...
			  @endif
                ...
                <tr class="editable" id="form-{{ $row->id }}">
                    ...
                    <td><a href="{{ $pageModule }}/show/{{$row->id}}"  onclick="ajaxViewDetail('#shopfegrequeststore',this.href); return false; "  class="btn btn-xs btn-green tips" title="Product Details">Details</a></td>
                    @if($setting['view-method']=='expand')
					<td><a href="javascript:void(0)" class="expandable" rel="#row-{{ $row->id }}" data-url="{{ url('shopfegrequeststore/show/'.$id) }}"><i class="fa fa-plus " ></i></a></td>
					@endif
                    ...
                    <td>
                        @if($row->inactive == 0)
                        <a href="{{ URL::to('shopfegrequeststore/popupcart/'.$row->id) }}">Add To Cart</a>
                    @else
                            Not Avail.
                        @endif
                    </td>
