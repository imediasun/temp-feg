    ...
    <div class="col-md-8"><label style="display: inline-block;">Please select a list to Review:</label>
        <select class="select3 selected_vendor" id="selected_vendor" style="width:60%;">
            ...
            @foreach($vendors_list as $vendor)
                <option vendor-id="{{ $vendor->vendor_id }}" @if($importVendorListId == $vendor->id) @if(empty($resetOmit)) selected @endif  @endif value="{{ $vendor->id }}">{{ $vendor->vendor_name.'  '.date('Y-m-d h:s',strtotime($vendor->email_recieved_at)) }}</option>
            @endforeach
        </select>
    </div>
    <div class="col-md-4">
